...
from data.clients import FMPAccessError, FundamentalsClient, MarketDataClient, _fmp_filings, _fmp_financials_snapshot, _fmp_get, _fmp_request, _sec_fundamentals_snapshot
...
def test_sec_fundamentals_snapshot_extracts_key_metrics(monkeypatch):
    monkeypatch.setattr("data.clients._sec_company_map", lambda: {"MSFT": "0000789019"})
    monkeypatch.setattr(
        "data.clients._sec_json",
            ...
            "facts": {
                "us-gaap": {
                    "Revenues": {
                        "units": {
                            "USD": [
                                {"fy": 2025, "fp": "FY", "end": "2025-06-30", "val": 1000.0},
                                {"fy": 2024, "fp": "FY", "end": "2024-06-30", "val": 900.0},
                            ]
                        }
                    },
                    "GrossProfit": {"units": {"USD": [{"fy": 2025, "fp": "FY", "end": "2025-06-30", "val": 600.0}]}},
                    "OperatingIncomeLoss": {
                        "units": {
                            "USD": [
                                {"fy": 2025, "fp": "FY", "end": "2025-06-30", "val": 300.0},
                                {"fy": 2024, "fp": "FY", "end": "2024-06-30", "val": 240.0},
                            ]
                        }
                    },
                    "NetCashProvidedByUsedInOperatingActivities": {
                        "units": {"USD": [{"fy": 2025, "fp": "FY", "end": "2025-06-30", "val": 320.0}]}
                    },
                    "PaymentsToAcquirePropertyPlantAndEquipment": {
                        "units": {"USD": [{"fy": 2025, "fp": "FY", "end": "2025-06-30", "val": 80.0}]}
                    },
                }
            }
        },
    ...
    snapshot = _sec_fundamentals_snapshot("MSFT", "test@example.com")

    assert snapshot is not None
    assert snapshot["source"] == "sec_edgar_fallback"
    assert round(snapshot["revenue_growth_yoy"], 2) == round((1000.0 / 900.0 - 1) * 100, 2)
    assert snapshot["ebit_margin"] == 30.0


def test_peer_data_adapter_appends_damodaran_benchmark(monkeypatch):
    monkeypatch.setattr("data.peer_data._fmp_stock_peers", lambda ticker, api_key: ["GOOGL", "ORCL"])
    monkeypatch.setattr("data.peer_data._damodaran_industry_snapshot", lambda sector, industry: {
        "matched_industry": "Software (System & Application)",
        "forward_pe": 24.0,
        "ev_ebitda": 16.0,
        "source": "damodaran_compdata",
    })

    rows = PeerDataAdapter().get_peer_snapshot("MSFT")

    assert any(row["ticker"] == "INDUSTRY_MEDIAN" for row in rows)
    assert any(row["source"] == "damodaran_compdata" for row in rows)
...
def test_filing_snippet_from_text_rejects_xbrl_noise():
    helper = __import__("data.clients", fromlist=["_filing_snippet_from_text"])._filing_snippet_from_text
    snippet = helper(
        "http://fasb.org/us-gaap/2025#DerivativeAssets us-gaap:DebtSecuritiesMember "
        "0000789019 2025-12-31 2024-07-01 2024-12-31 "
        "http://fasb.org/us-gaap/2025#DerivativeLiabilities"
    )
