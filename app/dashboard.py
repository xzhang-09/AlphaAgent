...
VERSION_TRACKED_FILES = [
    ...
    PROJECT_ROOT / "data" / "clients.py",
    PROJECT_ROOT / "data" / "peer_data.py",
    PROJECT_ROOT / "data" / "market_data.py",
    PROJECT_ROOT / "engine" / "valuation_model.py",
    PROJECT_ROOT / "pipelines" / "research_graph.py",
    PROJECT_ROOT / "agents" / "memo_agent.py",
    PROJECT_ROOT / "utils" / "source_summary.py",
]


def app_code_version() -> str:
    digest = hashlib.sha256()
    for path in VERSION_TRACKED_FILES:
...
def _run_research_from_sidebar(include_feedback: bool = False) -> None:
    raw_ticker = st.session_state.get("_raw_ticker", "")
    ...
    if not is_valid_ticker_format(effective_ticker):
        st.session_state["result"] = None
        st.session_state["run_error"] = "Ticker format looks invalid. Use symbols like AAPL, 0700.HK, 005930.KS, or JPY=X."
        return

    analyst_feedback = {}
    if include_feedback:
        analyst_feedback = {
            "bull_case": st.session_state.get("_feedback_bull_case", ""),
            "bear_case": st.session_state.get("_feedback_bear_case", ""),
            "catalysts": st.session_state.get("_feedback_catalysts", ""),
            "notes": st.session_state.get("_feedback_notes", ""),
        }
...
if "run_error" not in st.session_state:
    st.session_state["run_error"] = None
if "_raw_ticker" not in st.session_state:
    st.session_state["_raw_ticker"] = ""
if "_debug_mode" not in st.session_state:
    st.session_state["_debug_mode"] = False

current_app_version = app_code_version()
previous_app_version = st.session_state.get("_app_code_version")
if previous_app_version and previous_app_version != current_app_version:
    st.session_state["result"] = None
    st.session_state["run_error"] = None
st.session_state["_app_code_version"] = current_app_version

with st.sidebar:
    st.header("Research Controls")
    raw_ticker = st.text_input("Ticker", key="_raw_ticker")
    ...
    st.toggle("Debug mode", key="_debug_mode", help="Show raw JSON payloads for debugging.")
    st.button("Run Research", type="primary", on_click=_run_research_from_sidebar)
    if st.session_state.get("result"):
        st.markdown("---")
        st.caption("Refine the generated memo with targeted analyst feedback.")
        render_feedback_panel()
    st.button(
        "Regenerate Memo With Feedback",
        on_click=_run_research_from_sidebar,
        kwargs={"include_feedback": True},
        disabled=st.session_state.get("result") is None,
    )
