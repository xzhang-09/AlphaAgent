...
def _show_debug_payload() -> bool:
    return st.session_state.get("_debug_mode", False)
...
def _format_number_metric(value, *, suffix: str = "", decimals: int = 1) -> str:
    if value is None:
        return "N/A"
...
def render_signals_panel(result: dict, to_display) -> None:
    ...
    col2.metric(
        ...
        sum(
            1
            for key in ("event_signals", "fundamental_signals", "valuation_signals", "market_signals")
            if signals.get(key)
        ),
    )
...
def render_valuation_panel(result: dict, to_display) -> None:
    ...
    top = st.columns(4)
    top[0].metric("Current Price", _format_number_metric(valuation.get("current_price"), decimals=2))
    top[1].metric("Forward P/E", _format_number_metric(relative.get("forward_pe"), suffix="x"))
    top[2].metric("Peer P/E", _format_number_metric(relative.get("peer_forward_pe"), suffix="x"))
    top[3].metric("Price Percentile (6M)", _format_number_metric(valuation.get("historical_percentile"), decimals=0))

    if valuation.get("support_summary"):
        st.caption(valuation["support_summary"])
    ...
    elif not valuation_supported:
        st.caption("Scenario analysis is unavailable until current and benchmark valuation multiples are present.")

    st.markdown("**Sensitivity Table**")
    sensitivity_table = result["valuation_output"].get("sensitivity_table", [])
    if sensitivity_table:
        st.dataframe(pd.DataFrame(sensitivity_table), use_container_width=True, hide_index=True)
    else:
        st.caption("Sensitivity analysis is unavailable for this run.")
...
def render_evaluation_panel(result: dict, to_display) -> None:
    ...
    top = st.columns(3)
    top[0].metric("Auto Checks Passed", "Yes" if evaluation.get("passed") else "No")
