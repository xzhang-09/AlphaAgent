    ...
    def get_financials(self, ticker: str) -> dict[str, Any]:
        ...
        def _fetch() -> dict[str, Any]:
            ...
            payload = {
                ...
                "source_detail": "Yahoo fallback because FMP stable fundamentals were unavailable for this run.",
                "source_warning": (
                    "Financial Modeling Prep fundamentals were unavailable or rejected; using weaker Yahoo live fallback."
                    if self.api_key
                    else "Financial Modeling Prep key not configured; using Yahoo live fallback."
                ),
                "data_quality_flags": [
                    flag
...
class VectorStoreClient(BaseAPIClient):
    def __init__(self) -> None:
        super().__init__(api_key_env="VECTOR_DB_API_KEY")
        settings = get_settings()
        self.vector_db = settings.vector_db
        self.vector_db_path = str(settings.vector_db_path)

    def get_config(self) -> dict[str, str]:
        return {"vector_db": self.vector_db, "vector_db_path": self.vector_db_path}
...
def _fetch_json(urls: list[str], params: dict[str, Any]) -> dict[str, Any]:
    headers = {"User-Agent": "Mozilla/5.0 AlphaAgent"}
    last_error: Exception | None = None
    for url in urls:
        try:
...
def _sec_filings(ticker: str) -> list[dict[str, Any]]:
    cik = _sec_company_map().get(ticker.upper())
    if not cik:
        return []
    headers = {"User-Agent": "AlphaAgent research assistant support@example.com"}
